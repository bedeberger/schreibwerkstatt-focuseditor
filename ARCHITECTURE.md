...
- [ContentView.swift](schreibwerkstatt-focuseditor/ContentView.swift) — Auth-State-Maschine: `.unknown/.validating` → Loading/Login, `.signedOut` → Login, `.signedIn` → `EditorHostView`. Der Host prüft `editorBundle.state` (ready → Editor, refreshing → Loading, failed → Retry-UI), bindet das Fenster für den Kiosk (`WindowAccessor`), steuert Auto-Kiosk-beim-Start und die Auto-Hide-Toolbar (Hover-Streifen + Offset + `allowsHitTesting`), und blendet das `PagePickerOverlay` ein.
- [AppToolbar.swift](schreibwerkstatt-focuseditor/AppToolbar.swift) — `BookPicker` · Breadcrumb (Kapitel › Seite) · ⌘O-Button · `WritingStatsLabel` · `SyncStatusLabel` (Konflikte orange) · Überlauf-Menü (Darstellung + Abmelden). Hintergrund: `WindowDragArea` + `.ultraThinMaterial` + `BrandColor`. Im Kiosk/nativen Vollbild ausgeblendet.
- [KioskFullscreen.swift](schreibwerkstatt-focuseditor/KioskFullscreen.swift) — `@MainActor ObservableObject` + `WindowAccessor`. Speichert Fensterzustand, entfernt Chrome, blendet via `NSApp.presentationOptions = [.hideDock, .hideMenuBar]` Dock **und** Menüleiste aus, deckt den Screen, installiert einen Escape-Monitor (keyCode 53). Verwaltet die Ampel-Buttons und den `trafficLightInset` (dynamischer Toolbar-Einzug). Beobachtet native Vollbild-Notifications.
